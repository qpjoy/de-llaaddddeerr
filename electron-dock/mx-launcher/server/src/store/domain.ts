...
function siteSlotDeploymentPhases(
  ...
  const domesticEgressProxyReadinessCommand = [
    ...
    'if command -v ss >/dev/null 2>&1; then ss -ltnp 2>/dev/null | grep ":7890" || true; fi',
    'if command -v curl >/dev/null 2>&1; then curl -fsSIL --http1.1 --max-time 20 --proxy http://127.0.0.1:7890 https://www.gstatic.com/generate_204 >/tmp/mx-egress-probe.headers || { echo "blocked: generic HTTPS is not reachable through local egress proxy"; curl -Iv --http1.1 --max-time 20 --proxy http://127.0.0.1:7890 https://www.gstatic.com/generate_204 || true; if command -v journalctl >/dev/null 2>&1; then journalctl -u mihomo-client -n 80 --no-pager -l || true; fi; exit 1; }; sed -n "1,6p" /tmp/mx-egress-probe.headers; curl -sSI --http1.1 --max-time 20 --proxy http://127.0.0.1:7890 "https://auth.docker.io/token?service=registry.docker.io" >/tmp/mx-docker-auth-probe.headers || { echo "blocked: Docker auth endpoint is not reachable through local egress proxy"; curl -Iv --http1.1 --max-time 20 --proxy http://127.0.0.1:7890 "https://auth.docker.io/token?service=registry.docker.io" || true; if command -v journalctl >/dev/null 2>&1; then journalctl -u mihomo-client -n 80 --no-pager -l || true; fi; exit 1; }; sed -n "1,6p" /tmp/mx-docker-auth-probe.headers; curl -sSI --http1.1 --max-time 20 --proxy http://127.0.0.1:7890 https://registry-1.docker.io/v2/ >/tmp/mx-registry-probe.headers || { echo "blocked: Docker registry is not reachable through local egress proxy"; curl -Iv --http1.1 --max-time 20 --proxy http://127.0.0.1:7890 https://registry-1.docker.io/v2/ || true; if command -v journalctl >/dev/null 2>&1; then journalctl -u mihomo-client -n 80 --no-pager -l || true; fi; exit 1; }; sed -n "1,6p" /tmp/mx-registry-probe.headers; else echo "warning: curl missing; skip registry proxy probe"; fi',
