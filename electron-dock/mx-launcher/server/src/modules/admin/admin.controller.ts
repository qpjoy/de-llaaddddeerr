...
function domesticRuntimeConfigApplyScript(config: SiteSlotDomesticRuntimeConfig): string {
  ...
  return [
    ...
    `public_bootstrap_health_url=${shellQuote(publicBootstrapHealthUrl)}`,
    ...
    'command -v curl >/dev/null 2>&1 || { echo "blocked: curl is required for public HTTPS certificate verification" >&2; exit 1; }',
    ...
    'mx_public_https_verify() { attempt=1; while [ "$attempt" -le 10 ]; do if curl -fsS --connect-timeout 5 --max-time 15 "$public_bootstrap_health_url" >/dev/null; then echo "Domestic public HTTPS probe passed: $public_bootstrap_health_url"; return 0; fi; sleep 3; attempt=$((attempt + 1)); done; echo "failed: public HTTPS bootstrap or its system-trusted certificate is not ready at $public_bootstrap_health_url" >&2; echo "configure official Compass nginx to terminate this hostname, mirror the strict allowlist, and proxy over ${MX_DOMESTIC_PUBLIC_GATEWAY_NETWORK:-compass-gateway_default} to mx-domestic-edge:8088" >&2; return 1; }',
    './manage.sh up',
