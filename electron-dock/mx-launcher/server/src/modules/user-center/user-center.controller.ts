...
export class UserCenterController {
  ...
  async systemSubscriptions(@Headers(INTERNAL_OPS_TOKEN_HEADER) opsToken: string | undefined) {
    assertInternalOpsToken(opsToken);
    return { catalog: await buildSystemSubscriptionCatalog(this.store) };
  }
  ...
  async revealSystemSubscription(
    ...
    const directIp = systemSubscriptionDirectUrl(item, account.authToken);
    const domain = await systemSubscriptionDomainUrl(this.store, account, account.authToken);
    return {
      subscription: {
