  ...
  },
  {
    ...
    assert: (body) => {
      ...
      return body?.plan?.kind === 'oversea'
        ...
        && prepareAccess?.commands?.some((command) => command.includes('ln -sfnT /opt/mx/state/hysteria2-access-stack/.env /opt/mx/releases/oversea-access-stack/__release_revision__/.env'))
        && prepareAccess?.commands?.some((command) => command.includes('ln -sfnT /opt/mx/state/hysteria2-access-stack/config /opt/mx/releases/oversea-access-stack/__release_revision__/config'))
        && prepareAccess?.commands?.some((command) => command.includes('ln -sfnT /opt/mx/state/hysteria2-access-stack/data /opt/mx/releases/oversea-access-stack/__release_revision__/data'))
        ...
        && configureAccess?.commands?.some((command) => (
          command.includes(`HY2_EXPORT_BASE_URL=${state.overseaExportBaseUrl}`)
