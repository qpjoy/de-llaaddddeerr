...
const routingMode = ['internal-mihomo', 'cn-direct', 'global'].includes(selectedPolicy?.routingMode)
  ? selectedPolicy.routingMode
  : 'cn-direct';
const reservedInternalCidrs = Array.isArray(selectedPolicy?.reservedInternalCidrs)
  ? selectedPolicy.reservedInternalCidrs.map((cidr) => cleanCell(cidr, 'reservedInternalCidrs', true)).filter(Boolean).join(',')
  : '10.88.0.0/16,10.89.0.0/16,10.90.0.0/16,10.91.0.0/16';
...
const env = {
  ...
  TUNNEL_RESERVED_INTERNAL_CIDRS: reservedInternalCidrs,
  TUNNEL_DOMESTIC_GATEWAY_IP: cleanCell(selectedPolicy?.domesticGatewayIp ?? '10.88.0.1', 'domesticGatewayIp', true),
  TUNNEL_DNS_PATH: cleanCell(selectedPolicy?.dnsPath ?? 'wg-relay-internal-dns', 'dnsPath', true),
