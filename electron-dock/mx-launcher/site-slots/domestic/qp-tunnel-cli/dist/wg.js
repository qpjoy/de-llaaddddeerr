...
exports.runWgCli = runWgCli;
const node_child_process_1 = require("node:child_process");
const node_fs_1 = require("node:fs");
const node_path_1 = require("node:path");
const packageRoot = (0, node_path_1.resolve)(__dirname, '..');
const scriptCandidates = [
    (0, node_path_1.resolve)(packageRoot, 'resources/wireguard.sh'),
    (0, node_path_1.resolve)(packageRoot, '../../../scripts/wireguard.sh'),
];
...
async function runWgCli(args, ctx) {
...
function detectInstalledRole(command, instance) {
    const serverInstalled = (0, node_fs_1.existsSync)(`/etc/qp-wireguard/server/${instance}/server.env`);
    const clientInstalled = (0, node_fs_1.existsSync)(`/etc/qp-wireguard/client/${instance}/client.env`);
    if (serverInstalled && clientInstalled) {
        process.stderr.write(`Both WireGuard roles are installed for instance "${instance}".\n` +
    ...
    if (clientInstalled)
        return 'client';
    process.stderr.write(`No managed WireGuard instance "${instance}" is installed. Pass --server/--client or install/enroll it first.\n`);
    process.exit(1);
}
function resolveScript() {
    ...
    process.stderr.write(`Could not find wireguard.sh. Expected ${scriptCandidates[0]} in the npm package.\n`);
    process.exit(1);
}
function runScript(script, scriptArgs, role) {
