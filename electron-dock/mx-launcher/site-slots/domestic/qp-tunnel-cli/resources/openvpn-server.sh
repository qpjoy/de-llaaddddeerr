...
load_env() {
	...
	source "$QP_OPENS_ENV"
	# Instances installed before peer-mesh support do not carry this field.
	QP_OPEN_CLIENT_TO_CLIENT="${QP_OPEN_CLIENT_TO_CLIENT:-false}"
}
...
# Reconfigure only the directives owned by the peer-mesh toggle. Keeping the
# rest of server.conf byte-for-byte avoids silently changing an established
# endpoint, cipher policy, runtime or any local operator adjustment.
render_reconfigured_server_config() {
	local peer_mesh="$1"
	awk '
		/^# BEGIN qp-tunnel-cli peer-mesh$/ { managed = 1; next }
		managed && /^# END qp-tunnel-cli peer-mesh$/ { managed = 0; next }
		managed { next }
		/^[[:space:]]*client-to-client([[:space:]]*(#.*)?)?$/ { next }
		{ print }
		END { if (managed) exit 42 }
	' "$QP_OPENS_CONFIG" || return 1

	if [[ "$peer_mesh" == true ]]; then
		# client-to-client is bypassed when OpenVPN offloads the data channel.
		# This narrow compatibility guard is understood since OpenVPN 2.3.3:
		# old builds ignore only disable-dco; DCO-capable builds disable it.
		cat <<'EOF'
# BEGIN qp-tunnel-cli peer-mesh
ignore-unknown-option disable-dco
disable-dco
client-to-client
# END qp-tunnel-cli peer-mesh
EOF
	fi
}

# Preserve every existing and future env field; only this command's one owned
# key is replaced. That makes upgrading an older installed instance safe.
render_reconfigured_server_env() {
	local peer_mesh="$1"
	awk -v value="$peer_mesh" '
		BEGIN { written = 0 }
		/^QP_OPEN_CLIENT_TO_CLIENT=/ {
			if (!written) {
				printf "QP_OPEN_CLIENT_TO_CLIENT=\047%s\047\n", value
				written = 1
			}
			next
		}
		{ print }
		END {
			if (!written) printf "QP_OPEN_CLIENT_TO_CLIENT=\047%s\047\n", value
		}
	' "$QP_OPENS_ENV"
}

# EXIT traps run after Bash may have unwound a function's local scope, so the
# transaction guard intentionally uses dedicated process-global state. Each CLI
# process owns one transaction and clears the state on completion.
reconfigure_transaction_guard() {
	# Make the guard single-shot before any rollback command can itself receive a
	# signal or exit non-zero.
	trap - EXIT HUP INT TERM
	local restore_ok=true
	local backup_dir="${QP_OPENS_RECONFIG_BACKUP_DIR:-}"
	local stage_dir="${QP_OPENS_RECONFIG_STAGE_DIR:-}"

	if [[ "${QP_OPENS_RECONFIG_LIVE_REPLACED:-false}" == true \
		&& "${QP_OPENS_RECONFIG_COMPLETE:-false}" != true ]]; then
		if [[ -n "$backup_dir" && -f "$backup_dir/server.conf" && -f "$backup_dir/server.env" ]]; then
			cp -p "$backup_dir/server.conf" "$QP_OPENS_CONFIG" 2>/dev/null || restore_ok=false
			cp -p "$backup_dir/server.env" "$QP_OPENS_ENV" 2>/dev/null || restore_ok=false
		else
			restore_ok=false
		fi

		if [[ "$restore_ok" == true \
			&& "${QP_OPENS_RECONFIG_RESTART_STARTED:-false}" == true \
			&& "${QP_OPENS_RECONFIG_WAS_RUNNING:-false}" == true ]]; then
			warn "Reconfigure was interrupted during restart; attempting to start the restored configuration."
			(restart_running_server) || warn "The previous files were restored, but the service still needs a manual restart."
		fi
	fi

	[[ -n "$stage_dir" ]] && rm -rf "$stage_dir" 2>/dev/null || true
	if [[ -n "$backup_dir" ]]; then
		if [[ "$restore_ok" == true || "${QP_OPENS_RECONFIG_COMPLETE:-false}" == true ]]; then
			rm -rf "$backup_dir" 2>/dev/null || true
		else
			warn "Automatic rollback could not restore both files; backup retained at $backup_dir"
		fi
	fi

	QP_OPENS_RECONFIG_BACKUP_DIR=""
	QP_OPENS_RECONFIG_STAGE_DIR=""
	QP_OPENS_RECONFIG_LIVE_REPLACED=false
	QP_OPENS_RECONFIG_COMPLETE=false
	QP_OPENS_RECONFIG_RESTART_STARTED=false
	QP_OPENS_RECONFIG_WAS_RUNNING=false
}
...
reconfigure_server() {
	...
	while [[ $# -gt 0 ]]; do
		case "$1" in
			--client-to-client)
				[[ -z "$peer_mesh" || "$peer_mesh" == true ]] \
					|| die "Pass either --client-to-client or --no-client-to-client, not both."
				peer_mesh=true
				shift
			;;
			--no-client-to-client)
				[[ -z "$peer_mesh" || "$peer_mesh" == false ]] \
					|| die "Pass either --client-to-client or --no-client-to-client, not both."
				peer_mesh=false
				shift
			;;
...
This changes only the OpenVPN userspace peer-mesh mode. It preserves the
endpoint, subnet, runtime, egress setting, PKI, CCD addresses, issued profiles,
firewall and sysctl state. A running server is restarted once; if that restart
fails, the previous config is restored and started again.

Enabling forms a full-trust mesh between every enrolled client's VPN address.
It disables server-side DCO so mesh forwarding is deterministic without adding
Linux forwarding or firewall rules. Disabling removes this userspace path; it
is not a firewall isolation policy if the host already routes tunnel traffic.
EOF
				return 0
			;;
	...
	[[ -n "$peer_mesh" ]] \
		|| die "Choose --client-to-client or --no-client-to-client."
	require_root
	require_linux
	load_env

	[[ -f "$QP_OPENS_CONFIG" ]] || die "Missing managed server config: $QP_OPENS_CONFIG"
	grep -q '^# Generated by qp-tunnel-cli open install\.' "$QP_OPENS_CONFIG" \
		|| die "$QP_OPENS_CONFIG is not a qp-tunnel-cli managed file; refusing to rewrite it."
	grep -q '^# Generated by qp-tunnel-cli open install\.' "$QP_OPENS_ENV" \
		|| die "$QP_OPENS_ENV is not a qp-tunnel-cli managed file; refusing to rewrite it."

	local stage_dir backup_dir was_running=false
	stage_dir="$(mktemp -d "$QP_OPENS_HOME/.reconfigure-stage.XXXXXX")"
	backup_dir="$(mktemp -d "$QP_OPENS_HOME/.reconfigure-backup.XXXXXX")"
	chmod 0700 "$stage_dir" "$backup_dir"

	if ! render_reconfigured_server_config "$peer_mesh" > "$stage_dir/server.conf"; then
		rm -rf "$stage_dir" "$backup_dir"
		die "Could not render the candidate server config; nothing was changed."
	fi
	if ! render_reconfigured_server_env "$peer_mesh" > "$stage_dir/server.env"; then
		rm -rf "$stage_dir" "$backup_dir"
		die "Could not render the candidate server environment; nothing was changed."
	fi
	chmod 0600 "$stage_dir/server.conf" "$stage_dir/server.env"

	if cmp -s "$stage_dir/server.conf" "$QP_OPENS_CONFIG" \
		&& cmp -s "$stage_dir/server.env" "$QP_OPENS_ENV"; then
		rm -rf "$stage_dir" "$backup_dir"
		info "Peer-mesh configuration is already $peer_mesh; no files changed and no restart was needed."
		return 0
	fi

	server_is_running && was_running=true
	if ! cp -p "$QP_OPENS_CONFIG" "$backup_dir/server.conf" \
		|| ! cp -p "$QP_OPENS_ENV" "$backup_dir/server.env"; then
		rm -rf "$stage_dir" "$backup_dir"
		die "Could not back up the current configuration; nothing was changed."
	fi

	# The EXIT guard covers an interrupt or write failure between the two
	# same-filesystem atomic renames. Normal restart failures take the explicit
	# rollback path below.
	QP_OPENS_RECONFIG_BACKUP_DIR="$backup_dir"
	QP_OPENS_RECONFIG_STAGE_DIR="$stage_dir"
	QP_OPENS_RECONFIG_LIVE_REPLACED=false
	QP_OPENS_RECONFIG_COMPLETE=false
	QP_OPENS_RECONFIG_RESTART_STARTED=false
	QP_OPENS_RECONFIG_WAS_RUNNING="$was_running"
	trap reconfigure_transaction_guard EXIT
	trap 'exit 129' HUP
	trap 'exit 130' INT
	trap 'exit 143' TERM

	QP_OPENS_RECONFIG_LIVE_REPLACED=true
	if ! mv -f "$stage_dir/server.conf" "$QP_OPENS_CONFIG" \
		|| ! mv -f "$stage_dir/server.env" "$QP_OPENS_ENV"; then
		die "Could not install the candidate configuration; the previous files were restored."
	fi

	QP_OPENS_RECONFIG_RESTART_STARTED=true
	if [[ "$was_running" == true ]] && ! (restart_running_server); then
		warn "The new configuration did not start; restoring the previous files."
		if ! cp -p "$backup_dir/server.conf" "$QP_OPENS_CONFIG" \
			|| ! cp -p "$backup_dir/server.env" "$QP_OPENS_ENV"; then
			die "Rollback files could not be restored from $backup_dir."
		fi

		if (restart_running_server); then
			QP_OPENS_RECONFIG_COMPLETE=true
			trap - EXIT HUP INT TERM
			reconfigure_transaction_guard
			die "Reconfigure failed; the previous configuration was restored and is running."
		fi

		QP_OPENS_RECONFIG_COMPLETE=true
		trap - EXIT HUP INT TERM
		reconfigure_transaction_guard
		die "Reconfigure failed. Previous files were restored, but the previous service also failed to start."
	fi
	...
	if [[ "$peer_mesh" == true ]]; then
		info "Peer mesh enabled: enrolled clients can reach each other's VPN addresses."
		info "Server DCO is disabled for deterministic userspace forwarding; firewall and sysctl state were not changed."
	else
		info "OpenVPN userspace peer mesh disabled. Firewall and sysctl state were not changed."
		warn "This is not a spoke-isolation firewall policy; existing kernel forwarding rules may still route tunnel traffic."
	fi
	[[ "$was_running" == true ]] \
		&& info "The running server was restarted once." \
		|| info "The server was inactive and was left inactive."
}
...
EOF
	...
	if [[ -f "$QP_OPENS_PKI/crl.pem" ]]; then
		echo "crl-verify $QP_OPENS_PKI/crl.pem"
	fi
	if [[ "$QP_OPEN_CLIENT_TO_CLIENT" == true ]]; then
		# OpenVPN 2.6+ can silently offload the data channel to the kernel, where
		# client-to-client has no effect. Keep peer mesh in OpenVPN userspace so
		# it works without adding host-wide forwarding or firewall rules. The
		# guard is understood by OpenVPN 2.3.3+ and allowlists this one option for
		# older binaries that do not know DCO.
		cat <<'EOF'
# BEGIN qp-tunnel-cli peer-mesh
ignore-unknown-option disable-dco
disable-dco
...
write_server_config() {
	...
	render_server_config > "$tmp"
	chmod 0600 "$tmp"
	mv -f "$tmp" "$QP_OPENS_CONFIG"
	info "Wrote $QP_OPENS_CONFIG"
}

# Cipher policy is rendered for the actual server runtime instead of being
# hidden behind ignore-unknown-option: silently dropping an active cipher rule
# would be unsafe. 2.5 replaced ncp-ciphers with data-ciphers, and RHEL 7 /
# Amazon Linux 2 still ship 2.4, where data-ciphers aborts startup.
render_server_ciphers() {
...
# data-ciphers is present. ignore-unknown-option has existed since 2.3.3;
# this profile's broader feature floor remains OpenVPN 2.4.
...
# Reconfigure already proved the server is running. Restart it in place so the
# host unit's enablement state and the Docker container identity/restart policy
# remain untouched; the regular lifecycle restart intentionally has broader
# start/enable semantics.
restart_running_server() {
	load_env
	if [[ "$QP_OPEN_RUNTIME" == docker ]]; then
		docker restart "$QP_OPENS_CONTAINER" >/dev/null
		info "Restarted container $QP_OPENS_CONTAINER in place"
	else
		systemctl restart "$QP_OPENS_UNIT"
		info "Restarted $QP_OPENS_UNIT in place"
	fi
	assert_server_running
}
...
status() {
	...
	if [[ "$QP_OPEN_CLIENT_TO_CLIENT" == true ]]; then
		if grep -qx '# BEGIN qp-tunnel-cli peer-mesh' "$QP_OPENS_CONFIG" 2>/dev/null \
			&& grep -qx 'ignore-unknown-option disable-dco' "$QP_OPENS_CONFIG" 2>/dev/null \
			&& grep -qx 'disable-dco' "$QP_OPENS_CONFIG" 2>/dev/null \
			&& grep -qx 'client-to-client' "$QP_OPENS_CONFIG" 2>/dev/null \
			&& grep -qx '# END qp-tunnel-cli peer-mesh' "$QP_OPENS_CONFIG" 2>/dev/null; then
			info "Peer mesh: enabled (OpenVPN userspace; server DCO disabled)"
		else
			info "Peer mesh: DRIFT (run 'open reconfigure --client-to-client')"
		fi
	else
		if grep -qx '# BEGIN qp-tunnel-cli peer-mesh' "$QP_OPENS_CONFIG" 2>/dev/null \
			|| grep -qx 'client-to-client' "$QP_OPENS_CONFIG" 2>/dev/null; then
			info "Peer mesh: DRIFT (run 'open reconfigure --no-client-to-client')"
		else
			info "Peer mesh: disabled (kernel firewall still determines routed peer access)"
		fi
	fi
	info ""
...
Usage:
  ...
  qp-tunnel-cli open reconfigure --client-to-client | --no-client-to-client
  qp-tunnel-cli open create <name> [--ip ADDR] [--out FILE] [--oversea]
  qp-tunnel-cli open reissue <name> [--out FILE]
