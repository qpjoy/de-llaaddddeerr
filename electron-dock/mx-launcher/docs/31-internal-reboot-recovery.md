...
**本次恢复的最终状态：最新业务库已启用，原 Ops Token、飞书配置和身份/凭据检查点已恢复，
host runner 已启动且健康检查通过。当前路径及日常 deploy 以
[mx-launcher.deploy.md](../mx-launcher.deploy.md) 为现场索引；旧目录和完整备份保留，不做清理。**
本文后续保留了各个历史中间阶段，“此时不要 deploy”等限制只适用于对应未完成阶段。
真实员工密码、飞书授权及原联网功能仍需实际验收。
...
尚未有检查点的既有安装，应在数据源及原凭据确认正确、服务正常时建立身份和 Secret 恢复记录。
**本次已由认证恢复脚本建立检查点，无需再运行一次性恢复。** 早期缺失的原 Ops Token 已从
最新 etcd 副本找回；没有备份的凭据仍不能通过重启凭空恢复。
后续保留的是现在集群实际使用的原值。健康重复执行不会轮换 Token，显式在环境或 `.env`
...
后续曾出现 Service 拒绝错误密码、正确凭据查询成功，却报“查询没有连接到已核实的
PostgreSQL Pod/数据库”。验证 SQL 的 `inet_server_addr()::text` 会保留 `/32` 或 `/128`，
无法与 Kubernetes 的裸 Pod IP 直接比较；现改用 `host(inet_server_addr())`。仍严格
比较实际 IP、数据库名和业务标记，不接受其它 Pod 或仅网段相同的地址。此处错误同样
只需更新后重跑 `--finish`；不重新切换数据。失败诊断分别报告 IP、库名或返回结构问题。
