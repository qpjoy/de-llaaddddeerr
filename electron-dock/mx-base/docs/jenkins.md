...
满足任意一条就 `manage.sh deploy jenkins`：
...
```bash
bash scripts/manage.sh deploy jenkins
```
...
```bash
bash scripts/manage.sh password jenkins    # 打印 admin 密码
bash scripts/manage.sh agent-cmd jenkins   # 打印接入构建 agent 的命令
bash scripts/manage.sh status
bash scripts/manage.sh stop jenkins        # 停服务；MXT 完全不受影响
```
...
[ADR-0001](adr/0001-shared-platform-services.md) 的硬约束立刻失效。
...
2. JCasC 是否被接受（`manage.sh logs jenkins` 里会打印解析错误）
