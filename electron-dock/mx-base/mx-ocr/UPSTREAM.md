...
- 本地适配：新增统一管理包装器、mx-ocr 名称、GPU 检查、默认全 IPv4 接口监听及资源限制；容器增加 `com.mx-base.app=mx-ocr` 标签；deploy 在镜像准备成功后再替换容器；替换前再次检查 GPU 归属，按容器 ID 优雅停止本服务旧容器（也涵盖切到 fast 时释放旧 vLLM 容器）；显示名称改为 MX OCR；诊断和测试使用实际绑定地址，支持受控内网监听。
