...
`deploy` 准备镜像并重新创建本应用容器；`start` 恢复已部署容器，不下载模型；`restart` 重启并等待健康。改变 .env（包括显卡）后用 `deploy`，不是 `start`。`stop` 保留容器、镜像、命名模型缓存卷，释放该服务运行中的 CPU/内存/显存；不会停止原 `knock-ocr-*` 或其他应用。若原服务仍占 GPU 2，会拒绝启动，需操作者选择迁移窗口或其他空闲卡。
...
默认 `http://127.0.0.1:8710`，Web UI 与 API 共用端口：

- `POST /api/ocr`：上传文件；`?engine=fast|quality` 显式选引擎。
- `GET /healthz`、`GET /api/info`、`GET /api/metrics`：就绪、配置与运行指标。
- `GET /api/docs`：完整接口说明。

保留上游行为，**本服务没有新增租户鉴权层，并有在线调参接口**。默认仅本机监听；Hub 跨容器/跨主机接入时通过已有受控内网入口或反向代理鉴权，不直接暴露公网。URL 抓取功能可配置 `FETCH_ALLOW_HOSTS` 白名单。修改监听地址需要同时核验访问控制。
