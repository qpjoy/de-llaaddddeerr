...
Alternative: --throughput-test selects up to 32 old files of 32-128 MiB each,
at most 2 GiB total, at 100 MiB/s. A 600-second soft budget is checked between
files; hard NFS can still block. Phase timings do not prove sustained capacity.
...
[[ $# == 2 ]] || { usage >&2; exit 2; }
case "$2" in --copy-test|--throughput-test) ;; *) usage >&2; exit 2 ;; esac
