...
当前版本核对和 `nas infra repair prepare` 已通过，报告尾号 `a0131341096e4ba9b9e3e61c3e2581dd`。最新 `mx-nas-part1-repair-copy-0096322ad3.service` 已回传完整成功：本次新增 181 个，已存在且核验通过 5,108 个，合计 5,289 个 / 2,892,300,834 字节（2.69 GiB）。复制不限速、不覆盖 NAS、不删除 SSD、不重启业务；仍是在线快照，不能清理。下一步为新增的[当前版本维护切换](operations/part1-repair-switch.md)，服务器尚未执行；此前失败证据保留在[修复记录](operations/no-ssd-fallback.md)。

用户最新顺序：**Part 1 完成 NAS 切换、业务验收、恢复基准登记和新 SSD 清理清单核验，保留 SSD 待日后删除；然后再将 Part 2 推进到同样状态。** 不在 Part 1 在线补齐后就启动 Part 2。Part 1 新切换入口已实现，尚待现场回执；新基准登记、业务验收及适配本次差异的清理前核验仍待完成。Part 2 独立切换/回收执行能力仍未实现；详见 [完整迁移终点与当前阻塞](operations/no-ssd-fallback.md#完整迁移终点与当前阻塞)。
