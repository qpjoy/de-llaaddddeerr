...
用户最新要求：先复制到 NAS，再切换和恢复任务；今晚先 po_infra，后台预复制不设四小时退出，优先完成单卷，允许 10–30 分钟维护窗口，验收后已授权回收对应 SSD 旧媒体，另一卷作为次晚 Part 2。复制期间保留全部原数据；任何阶段均不删除整个 named volume。实际命令与当前实现边界见 [两晚执行说明](../operations/two-night-migration.md)。
