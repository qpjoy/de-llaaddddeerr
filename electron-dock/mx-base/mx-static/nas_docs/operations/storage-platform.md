...
900 GB / 10 MB/s 是 25 小时；表中使用 GiB/MiB。1 Gb/s 链路理论约 119 MiB/s，实际有效吞吐更低；已取得短时复制结果，但尚未取得链路协商速率和整卷持续磁盘吞吐，不能承诺达到某一档。
...
1. 两卷小样本及 delta 1.513 GiB 吞吐测试均已通过；rsync 55.401 MiB/s，完整过程 31.708 秒。不重复试拷，先 [在线预复制 po_infra](two-night-migration.md)，全部 tmp 和源文件保留；单遍算术外推 2.55 小时，持续速率以整卷执行为准。
2. 用户允许 po_infra 10–30 分钟维护窗口，条件授权验收后回收该卷旧 raw_media，次晚再处理 delta。完整校验须尽量前移；当前预复制退出 0 不授权脚本自动切换/回收。
3. 并行补充 [NAS 后端健康、快照/备份信息](nas-health-and-oss.md) 及本机 `nas-audit.sh network` 链路快照。用户确认目前无独立备份，不能把 NAS 历史稳定当作可恢复备份。
4. 切换前在隔离目录/栈验证原生 NFS 子目录挂载、实际容器权限和启动故障；确定自动恢复责任，完成最终一致校验及回滚方案。Kubernetes 作为后续统一存储管理方向，本轮不新装 CSI、Operator 或迁移其他数据库。
