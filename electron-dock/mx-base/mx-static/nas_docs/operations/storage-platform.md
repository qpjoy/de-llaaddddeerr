...
1. 两卷 [小批复制](sample-copy.md) 均已通过，共 16 个文件。下一步仅运行单卷最多 2 GiB 的 `--throughput-test`，100 MiB/s 限速和 600 秒软预算，区分复制、fsync、哈希与其他开销；不自动进入全量迁移。
2. 结合短时测试、链路速率与 NAS 业务余量，再确定全量预复制限速及四小时目标是否可行；短时缓存影响和完整校验成本不能忽略。首次仍复制所有 tmp，保留源卷。
