...
> **当前暂停执行本页清理/切换命令。** 9 月 24 日 11:04 新版重建再次丢失 NAS 子挂载，十个服务已确认使用 SSD。本页记录此前成功验收及当时的操作；旧清单已取消选用，原文件保留。先按 [本次现场与只读诊断](2026-09-24-second-fallback.md) 处理，不能沿用历史 `reclaim_ready=true` 删除现在的数据。
