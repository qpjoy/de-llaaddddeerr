...
**最新代码：** 已实现 [独立 NAS 项目恢复与按需重建](media-runtime.md)，仅改 mx-static；`media-v1` 日常恢复不读取应用配置/目录、不绑定历史镜像或 ID。`nas infra start` 包含已有 PostgreSQL/Redis 和媒体服务，按当前容器依赖及健康条件补启动，已运行的跳过。正常恢复/部署不删除 NAS、数据库或队列；迁移清理是独立流程。服务器需先一次登记，再安装/检查。下文包含旧迁移适配及设计沿革；`redeploy` 为旧同版本入口，当前应用配置重建使用 `deployment recreate --maintenance [--build]`。未修改或拦截 po-infra 原发布入口。

**2026-09-24 现场更新：** infra 已完成当前版本修复切换，十个媒体服务实际 NFS 挂载匹配。最新诊断确认服务器仍登记旧报告 `830225…`，而当前部署文件、渲染配置及容器均匹配新报告 `33e5abb…`；恢复启用受旧登记阻止，尚无新清理前核验回执。先同步正确登记，再按 [恢复与只读清理前核验](part1-union-reclaim.md) 处理；不重复切换，不改报告摘要。用户另要求正常业务发布无须重新审核迁移基准，见下方待实施的解耦设计。
...
| 补启动 | `infra start` / `recover part1` | 新建临时单元，按依赖补启动已有 PostgreSQL/Redis 和 NAS 媒体服务；不重建/同步/删除 |
...
**旧迁移适配的限制：** 正式升级业务版本、修改 Compose/env 或更换数据库容器，仍可能使迁移/清理及旧同版本重建入口因历史基准不符而拒绝。新 `media-v1` 日常恢复已不使用这些应用快照检查；它也不会因普通配置变化自行构建、重建或停止正在运行的业务。显式当前版本重建另走 `deployment recreate`。

## 日常发布与存储约束解耦（待实施）

本节为前序设计记录；用户后续明确仅改 mx-static，并允许仅在构建/重建时依赖应用部署。已落实的范围、命令及未覆盖的外部发布入口，以 [媒体恢复与按需重建](media-runtime.md) 为准；动态改名/新增媒体角色等通用发布能力仍未实现。

2026-09-24 用户明确要求：业务可自由修改与存储无关的配置、升级版本、重建容器；NAS 管理对日常使用应透明，不能每次发布都要求 mx-static 更新整份应用基准。本节记录设计方向，未改变现有恢复/清理代码，也不表示原发布入口已受保护。

`/home/lcy/test/Delta/mx_data` 是当前部署工作目录，并不是 MX-H2I 终端用户的个人配置目录。现有适配器从这里读取 Compose 和 `.env.ghcr`，保存字节摘要与渲染快照；`manage.operation()` 又复用了 `Cutover.config_guard()` 和固定容器身份检查，所以正常发布也可能被恢复工具拒绝。迁移历史应保留，但不应永久充当业务版本锁。

| 变化 | 长期应有行为 |
| --- | --- |
| 镜像、与存储无关的环境变量、端口、业务启动命令 | 由业务发布流程管理；不与迁移时的值比较，不重新申请 NAS 迁移验收 |
| 重建导致容器 ID 改变、扩缩容或服务改名 | 按当前授权部署中的媒体角色核对所有实例；正确挂载即可，不永久固定十个名字及旧 ID |
| NAS 导出、卷身份、媒体挂载路径、读写角色或存储访问身份变化 | 属于存储配置变更，核对新声明、访问能力和必要的数据迁移，不能按普通版本变化忽略 |
| 缺少 NAS 子挂载、同名卷实际为普通本地卷 | 在受控发布的启动之前阻止该媒体服务；不由监测器静默重建、不退回 SSD |
| 日后删除旧 SSD 文件 | 独立执行当时的存储/消费者与逐文件核验；历史“可回收”状态不能直接授权删除 |

职责按以下边界实施：

1. **持久存储声明。** mx-static 管理 NAS 目标身份、external 卷、容器媒体路径、只读/可写角色和 nocopy；声明不包含应用镜像、登录配置、业务启动命令或迁移时的容器 ID。现有 `part1.storage.json` 已分离这些存储字段，但仍是十个服务的静态覆盖，尚不足以满足上述动态部署要求。部署工作目录可作为可更新的定位信息，不能当作数据身份或永久写死在通用恢复代码中。
2. **业务部署。** 用户原来的发布流程继续拥有镜像、应用配置、初始化/升级策略和编排；一次接入后必须自动携带并检查平台存储声明，无须用户每次记住额外参数或复制旧报告 override。服务新增、改名和扩缩容时，从当前授权部署的媒体声明产生对应挂载，发布前检查最终配置，创建后核对实际卷。仅依赖用户自愿加 `-f` 或环境变量不足以防遗漏。
3. **开机恢复。** 使用独立、持久的 NAS 存储意图和当前部署，核对实际 NFS 卷与媒体消费者。正常镜像/env/ID 变化不构成恢复失败；一次恢复操作仍须固定当次目标并在启动前复核，避免与正在发布的进程竞争。保留项目/全局暂停，不启动任意扫描到的未知容器、旧版本孤儿容器或已缩容实例。只补启动允许恢复的现存容器；缺容器/卷时交给当前部署的恢复流程，不凭历史报告拉旧镜像、重建应用或初始化数据库。Postgres/Redis 仍由业务平台管理，NAS 工具不接管其版本和生命周期。
4. **迁移与清理证据。** 迁移报告保持不可变，继续证明当时复制、停写、切换的事实；正常发布不改写报告来“通过校验”。日后清理以原始清单、NAS 对应文件、实际消费者和当前存储约束重新核验，不让旧报告成为永久应用版本锁；任何源/目标文件变化仍需处理，不能通过解除应用绑定一并放宽文件删除保护。当前已实现的清理入口仍保留原严格检查，需在独立恢复路径与发布约束验证完成后才能调整。

Compose 的 `external: true` 将卷的创建/管理交给应用外部，缺卷时会报错。但它不校验同名卷一定是 NFS，且只有实际部署配置使用了该声明才有效。因此还要核对卷选项和实际挂载，并接入真正执行的发布入口。[Docker Compose 外部卷说明](https://docs.docker.com/reference/compose-file/volumes/#external)

接入后，正常发布无需人工维护 NAS 历史基准；NAS 缺失/不可达时仍不能承诺应用无感可用。自由修改应用也不能包括无声移除受保护媒体的 NAS 约束。管理员 root 直接调用 Docker 能绕过入口；不修改 po-infra、不接管 Docker API 的现有范围下，目前不能声称所有外部发布已被拦截。

实施顺序：先建立独立存储约束及隔离测试，再接入实际发布入口，然后替换开机恢复对历史应用快照的依赖。不能仅删除 `config_guard`/容器 ID 校验就宣布解耦完成。验收应覆盖：普通 env/镜像变更、重建/扩缩容、媒体服务改名、NAS 迟启动、缺卷/错误卷/遗漏挂载、维护暂停以及 Docker 元数据丢失；正常变化通过，存储错误在写入前被阻止。测试不重启当前生产服务、不重置 MX-H2I 账号或数据库，也不执行 SSD 清理。
