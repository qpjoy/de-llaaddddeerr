...
最新代码补充：[独立媒体恢复与按需重建](media-runtime.md) 已在 mx-static 实现，日常恢复不依赖应用配置；缺失媒体容器可由显式维护入口使用当前 Compose/构建重建，缺数据卷则拒绝创建空替代。服务器尚未采用本版，也未封住任意外部 Docker/原应用发布入口。

状态（2026-09-24）：本文区分现有能力和待实施的发布约束。服务器先确认十个媒体服务因重建遗漏覆盖而回到 SSD；随后补齐 5,289 个候选，再由 `mx-nas-part1-repair-switch-194cd1338d.service` 成功恢复到 NAS，新报告尾号 `33e5abb193d04e7595251a5e6a6046ae`。最终核验通过，数据库/Redis 保持原 ID，SSD 未删除。用户尚未全部检查业务；下一步为[恢复安装及只读清理前核验](part1-union-reclaim.md)。未进行断电/重装演练，未封住外部发布入口，不能宣布所有回退路径均已受控。
...
2. **受控发布必须携带存储声明。** 当前迁移修复核对十个 child mount、外部卷、gateway 只读、nocopy、父卷和 DB/Redis，再创建未启动容器，检查实际配置后才启动。长期发布应自动接入独立存储声明，并依据当前授权部署确定媒体消费者；业务镜像/env/启动命令不绑定历史迁移报告，见 [发布与存储解耦设计](unified-management.md#日常发布与存储约束解耦待实施)。检查失败不得重试为不带存储声明的命令。旧成功报告包含固定镜像及维护启动覆盖，不能直接拿它作为今后的正常版本发布配置。
...
当前能力仍有缺口：Part 1 维护切换已成功，Git 新基准登记和 UNION 清理前核验入口已更新，但安装/核验现场回执及业务验收仍待完成；外部发布入口防绕过尚未实现。Part 2 的 `precopy-only` 适配仅允许预复制，需独立审核其部署、镜像、启动行为和身份后补齐切换及回收流程。不能靠新增一个登记字段或手工改旧报告将这些步骤标记为通过。
