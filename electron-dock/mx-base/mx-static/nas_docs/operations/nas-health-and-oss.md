...
已知：客户端 NFS 读写、基础属性与 1.513 GiB 短测试通过。未知：NAS 品牌型号、实际文件系统、存储池/硬盘健康和快照。用户确认当前没有独立备份。NFS 是协议，不能从客户端 `df -T /mnt/nas` 推导 NAS 后端是 Btrfs/ext4/ZFS 或 RAID 健康。

用户最新决定：本轮先验证 NAS，暂不进行阿里云 OSS 备份，不采购资源、不上传文件；下方价格仅保留为历史预算，不作为本轮迁移前置条件。保留“目前无独立备份”的事实，NAS 健康检查不能替代备份。

当前先回传两端只读结果：NAS 端使用下方命令；mx-internal-server 的 mx-static 目录运行 `bash scripts/nas-audit.sh network` 和 `df -hT /data`。已通过的写入、小批校验和短时吞吐不重复。之后仍需实际容器权限、Docker NFS 子目录挂载及启动/故障恢复验证，才进入最终切换；这批只读输出不单独构成可切换/可回收证明。若预复制已经在运行，不为本次只读检查重启或强行中断它。
