...
## 当前检查点：第一卷预复制完成，开始在线完整校验

用户已回传第一卷 `precopy_pass_complete`、`last_exit_code=0` 和 `mx-nas-part1-po.service: Succeeded.`；03:04:04 至 06:14:39 完成，总大小 534,415,703,995 bytes，整轮等效约 44.57 MiB/s。**不要再为取消限速重跑这轮已完成的复制**。两项 ready 均为 false，下一步按 [在线完整校验](online-verification.md) 启动 SHA256 和文件集合检查，暂不开始第二卷、停写或清理。
...
先在业务在线时完成大部分复制和内容核验。第一卷复制已完成，现已补充 [完整在线 SHA256 清单工具](online-verification.md)，等待现场执行结果；**尚不能直接进入维护窗口**。对于约 497 GiB，如果把全量 NAS checksum 全放到停写窗口里，可能需要数小时，违反用户约束。

后续切换入口必须具备：预先完成的逐文件内容验证及源/目标身份清单；停写后重新枚举全部文件集合和稳定性标记，对新建/变化文件重新复制并核验；删除项显式隔离；任何身份/时间戳/外部写入不能解释时回到完整核验。不能只凭一次 rsync 成功、du 相同或秒级 mtime 相同就跳过内容验证。在线清单工具已实现，最终停写增量验收与切换/回收入口仍待实现和现场验证，不以在线报告或设计文字作为清理凭据。
