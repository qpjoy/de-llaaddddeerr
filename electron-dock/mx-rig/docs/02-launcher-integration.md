...
平台测试中心将来可以调用 Rig 测试 API并保存 evidence 引用；只接入有限摘要、状态与原始证据 URL。MX Rig 的 Agent 答复不能直接调用 Launcher gate evaluate，更不能给生产发布自动放行。

## 上线验证

在独立预发布环境验证 MX-H2I 登录、已有连接、断开与退出；Rig 缺席、停止、超时、部署、模型不可用、浏览器崩溃均不得改变其结果。比较 Launcher Deployment/Pod generation、端口、route/WG/PAC/NRPT 状态和登录 API 错误率。现网未执行这些检查前，不宣称现网集成已验收。
