...
// Windows statfs reports both inode fields as zero: it has no Unix inode
// accounting. Keep byte and entry quotas; do not mistake unsupported metrics
// for a full disk. A real zero free-inode count on Unix still fails closed.
export function hasInodeAccounting(filesystem, platform = process.platform) {
  return !(platform === 'win32' && Number(filesystem.files) === 0 && Number(filesystem.ffree) === 0)
}
...
export class ArtifactStore {
  ...
  async #write(
    ...
    if (hasInodeAccounting(filesystem) && (
