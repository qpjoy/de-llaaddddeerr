...
来源下拉的禁用条件是未取得调用凭据（`!key`），并非缺少独立的新闻发现能力。租户存在多把 Live Key 时必须在页面顶部“当前调用身份 → 我的 Key”手动选择。无可用 Key、身份请求失败、或成员不具有 `apikey.write` 都可能导致未取得凭据；查看身份区的具体原因。既有临时凭据接口只允许租户所有者/管理员使用所属租户 Key，不向只读成员扩权。

开通步骤：

1. 平台管理员进入“开放能力”，选择目标租户和调用者，搜索“新闻发现”或 `saved_records`。在“存量记录 · 按栏目”中按需启用“数据中心 · 新闻资讯”（`data_center_saved_records_news`）；财经、科技、本地新闻等类别也可能包含新闻，应分别按实际需要授权。批量管理整个租户可用“调用者 → 租户业务开通”。
2. 在“API Keys → 调整 Key 权限 → 数据域 / 来源范围”勾选已开通类别，点击“应用权限到原 Key”。保留原密钥；仅扩大调用者权限不会自动扩大旧 Key 的快照。
3. 回到新闻发现，在“当前调用身份”选择这把 Live Key，或点击“刷新调用身份 / 重新检查当前 Hub Key”。展开新闻来源并查询。

新闻 API 只要求有效 saved-record 类别授权，不要求单独的 `news.discovery`、`source_catalog` 或 `data.canonical-search` capability；最后一项是计量名称。未获得类别授权会返回 403；选项为空表示当前授权范围暂未发现已绑定目录的可读新闻，仍可不限目录查询。页面分别提示未取得 Key、成员权限不足、调用者未开通及 Key 未包含新权限；没有自动修改用户、授权、价格或登录配置。

验证：27 项身份、权限提示、新闻 SQL 与开放能力回归通过，构建和类型检查通过。隔离站点使用模拟租户身份、真实 Hub 临时凭据及 PGlite 验证多 Key 手动选择、调用者未开通、旧 Key 未勾选、已授权 Key 下拉与检索、只读角色继续拒绝，以及“新闻发现”查找授权类别；桌面 1440×1040、手机 390×844 均通过，未调用真实 Launcher 或生产数据。

### 12.7 下游完整接口调试

“接口调试”展示全部五个既有新闻 API：来源下拉、数据类别/目录元数据、新闻搜索/分页、文章详情、来源/类别统计。默认选择来源下拉，显式发送后显示真实 `key/value` 对照并可勾选填入 `catalogEntryIds`。类别使用 `categories[].id/label`；搜索返回的文章 `id` 可直接填入详情。目录 ID、文章 ID、类别代码不混用；固定 binding/timeField 枚举见合同。

每个接口提供 HTTP 方法/路径、参数来源说明、cURL/PowerShell/浏览器 fetch/Node.js 示例、真实响应、Request ID 和实际发送参数。复制示例只带 `<HUB_API_KEY>` 占位符，不泄露浏览器临时凭据；GET 示例不带请求体，只有搜索/详情/统计携带幂等键。元数据没有计量请求标识。

调试器在切换新闻列表、接口页或接口种类时保留状态，不自动发送。新查询条件使用独立标识，同条件重放/失败重试复用原标识；明确“新建请求标识”后再次发送才成为新请求。下一页复制已提交的搜索条件，仅替换 cursor；失败页可用原参数/原标识重试。读取目录和类别后更新筛选选项，不触发文章查询。调试响应与新闻列表的查询各自保留，公共授权、计量、登录和联网规则不变。

验证：33 项接口、调用示例、文档与 SQL 专项回归通过，构建与类型检查通过。隔离 PGlite 站点的浏览器验证覆盖五个接口、名称/ID 多选、复制示例不含密钥且不触发请求、固定条件翻页、失败页原参数/原幂等键重试、文章 ID 回填、切换视图保留状态和明确新建请求标识。桌面 1440×1040 与手机 390×844 无页面横向溢出或脚本异常；长 ID 表格在区域内滚动。没有执行生产查询或部署。
