...
### 已启用却返回成本凭证异常（2026-09-23 修复）

实际管理页显示详情/评论采用 CNY 采购价、搜索/正文采用 USD 采购价。旧准入逻辑会把同一上游另一请求的不同币种历史调用或未释放预占视作 `external_platform_cost_evidence_incomplete`，从而阻断没有正价钱包预占的请求，即使操作已启用且预算充足。此校验与画卷话题渲染无关；之前的同币种合成测试没有覆盖该组合。

修复后，Memory/PostgreSQL 的月度成本与预占按上游和原币种分别核算；管理预算展示的预占也按币种过滤。不做汇率换算，不改现有采购价、客户价、权限、余额或历史。同一个请求的调用/预占币种不一致，以及无效币种与相应账本的异常成本证据，仍然阻断；同币种采购/补贴预算继续生效。

新增测试先复现了同一个错误，再验证 CNY 的详情和评论可以与 USD 历史共存，并验证双币种预占、同币种预算上限、同一请求混币拒绝。相关 148 项回归通过。需要重新部署 Hub 后端才能生效；不需要数据库迁移、重新签发 Key 或重新发布价格。尚未部署或用生产 Key 发起付费复测。
