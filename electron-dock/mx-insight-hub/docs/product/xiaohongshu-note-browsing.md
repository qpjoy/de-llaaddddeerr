...
- 列表或 Mobile 点击笔记即请求详情，不再要求再次点击“获取完整正文”。优先选择当前 Key 已授权且运行可用的 `/notes/detail`；若该操作未就绪，则直接使用已授权、可用的原正文/标签接口。每次打开至多选择一种操作；发出请求后失败不会自动换接口重试。
- 正文、图片和指标置于弹窗前部，补充正文/阅读量、权限诊断、图片设置和交付证据折叠到“更多操作与请求记录”。保留一行计费及 5 秒间隔建议；不引入等待或缓冲策略。评论仍按点击独立加载。
- 详情顶部提供“正文与图片 / 评论”切换，评论不再藏在长图文末尾。首次明确点击“评论”请求一页 `/api/v1/data/xiaohongshu/notes/comments`；已加载、失败、切回及关闭重开均保留结果与请求身份，不自动再发。无授权或运行未就绪时直接显示原因；失败通过“重试评论”保留原标识，下一页单独请求。正文加载状态标明当前取得的是“正文与标签”还是“详情与阅读量”。
