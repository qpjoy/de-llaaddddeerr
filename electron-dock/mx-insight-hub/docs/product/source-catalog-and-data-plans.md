...
2026-09-23 增量规划见 [数据源目录与 Hub 实际接入对账](source-catalog-reconciliation-plan.md)：
本文的 Phase 1 人工覆盖字段继续保持兼容；后续拟新增基于能力/运行证据的接入摘要，
区分“平台至少一项能力接通”和“约定能力范围全部完成”。该增量规划尚未实施。

2026-09-24 已在本地实现记录级增量：[新闻发现与数据归类](news-discovery-design.md#12-目录可行性评估与首版落地)。入库确定来源绑定目录，历史记录可经规则/Agent 建议与审核补绑定；目录作为新闻检索维度。能力级接入对账、全库自动分类和真实覆盖审计仍未完成，未部署生产。
...
当前 related-data 关联使用目录规范名称与 alias 的 NFKC/大小写归一匹配；平台改名自动保留旧名
为 alias，名称/alias 的跨平台歧义由唯一所有权约束直接拒绝。显式 dataset/source binding 属于后续
relation 模型，不能把 ES 文档反向写成目录权威事实。
