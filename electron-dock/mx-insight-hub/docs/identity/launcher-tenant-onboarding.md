...
迁移 `078_api_key_vault.sql` 后，新签发 Key 的完整内容使用 AES-256-GCM 加密保存；鉴权仍使用原有 HMAC 哈希。加密密钥从现有 API Key pepper 按独立用途派生，部署必须保留原 pepper。列表、用量与诊断不返回密文或明文。

在 Key 列表点“查看完整 Key”，输入当前 Launcher 账号和密码。后端重新验证身份，并检查该账号对目标租户的 `apikey.write` 权限；普通登录会话不能换用其他账号查看。Admin Token 会话也需要一个有对应权限的 Launcher 账号。密码不保存，成功查看记录成员、Key ID 和时间，响应禁止缓存。

历史 Key 只有哈希，无法补算完整内容。需签发替代 Key、更新客户端后再撤销旧 Key；不会自动撤销线上旧 Key。Launcher 不可用时暂不能查看完整 Key，已有 API 调用不受此功能影响。
