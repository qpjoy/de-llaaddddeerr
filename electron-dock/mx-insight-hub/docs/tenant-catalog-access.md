...
- 会话和文档的产品授权改为一次按所属租户查询，返回每个调用者的独立授权组合；不再全量加载调用者或逐个查询授权。不缓存授权结果。
- 租户账单页不再等待其界面不使用的开放能力配置接口。
- PostgreSQL 普通 Key 认证由两次数据库往返合并为一次；演示 Key 的 ID 查询也直接完成同一认证查询。租户演示凭据仍额外实时检查成员身份与 membership，普通机器 Key 不经过 Launcher。
- 已绑定的 Hub 登录成员使用单条语句读取当前状态并更新必要的身份资料；角色同步与租户关系读取并行。Launcher 原有 introspection 缓存策略未改变。
- last_used_at / last_seen_at 最多每分钟写入一次，仅影响最后访问时间的显示精度。请求授权、撤销、状态与有效期检查仍逐次读取；调用计量和钱包流水不采样、不合并。
- 本项没有新增迁移。以上是查询链路与往返次数的改进，不是生产环境耗时实测；实际延迟还受数据库网络、连接池与服务负载影响。

## 套餐价格与授权

调用者绑定一个套餐版本，套餐可包含多个接口的计量价格。授权决定能否调用；价格表只决定是否扣费。已授权但未配置价格的操作免费，仍计入请求用量并受 Key、调用者、套餐额度限制。显式价格 0 同样免费；已配置的小红书价格继续按原规则结算。

部署时需执行 `080_unpriced_customer_operations.sql` 更新 PostgreSQL 计费触发器。无需重发 Key、重新分配套餐或修改已发布价目表；历史费用不追溯修改。价格表异常、数据库错误不会按免费处理。
