...
- 当前实时输出是有界第一页结果窗口，`sources[].hasMore` 说明来源有后续页；尚未实现跨来源连续游标／持久结果窗口，也不声称覆盖全网或全部页。`pageSize` 仅控制存量结果；实时社交每源 20 条、商品沿用既有合同页上限。原 Key 上限仍可拒绝不合策略的请求。
