...
### 初次向量化出现 search_index_backend_mismatch

先勾选「暂停新任务」并保存，保留历史任务（不要使用「停止回填」取消任务）。
该错误表示当前 ES 索引的分词来源与严格 HanLP 写入要求不一致或未登记，并不表示模型推理慢。
升级后，在「启用后台向量化」保持勾选时保存设置，会校验来源记录：
仅当初始具体向量索引为空、来源未登记且实测 HanLP 正常，才在排他写入锁下补登记。
不修改已有其他后端标记，不为非空索引伪造来源，不设置语料回填完成水位。
PG 中已经生成的向量保留。修复成功后解除暂停并保存；若任务已经进入失败状态，再点击「重试失败记录」。
非空或来源冲突的索引需要显式索引恢复，不能通过手工修改标记绕过校验。
