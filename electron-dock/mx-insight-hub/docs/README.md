...
| Area | Current state |
| --- | --- |
| Modular monolith API | Implemented: multiple tenants, tenant rename, consumers, one-time API keys, immutable per-key platform/capability entitlement snapshots, plan assignments, effective quota intersection, request idempotency, usage and health. Each consumer belongs to exactly one tenant. New snapshot keys default to zero permissions and explicitly select data-domain/platform, business-operation and compatible-interface-contract grants. `legacy_all` is opt-in; legacy dynamic keys remain explicitly identified for rotation. Newly granted capabilities require a newly issued key. |
...
| Night-All compatibility and direct migration | Implemented for the provider-neutral `/api/v1/data/search` facade and the three explicit `/api/v1/night-all/search/{raw,crawl,user-info}` migration routes. Migrated Xiaohongshu raw/search/detail and narrow single-user crawl/user-info shapes use separately gated Hub-native TikHub workflows; requests carrying historical `mxnc1` continuations, batch/channel/non-post and other unsupported shapes remain on the transitional Night-All path until migrated. Compatibility delivery does not desensitize, filter or truncate business data; only pagination controls are governed. Historical lineage retains complete parsed JSON and legacy raw strings, while Hub-native provider calls add restricted, bounded exact-byte archives. Request credentials stay out of responses, ordinary UI and logs. Repository presence is not proof that a rollout gate is active. |
...
| Data ingest and serving plane | Admin-token-only PostgreSQL/file source management, browser upload, allowlisted server-file paths, content observations, reusable immutable format rules, canonical records/revisions/tombstones, projection outbox, durable queues/cursors, interpretation-aware file idempotency, PostgreSQL external pull and import-run evidence are implemented. Shared Admin-managed `catalog.database_connections` profiles own transport credentials for fixed pipelines such as Night-All saved records; legacy inline source connections remain compatible. Backups containing either connection form are sensitive. `/shared_dir` directory watcher/landing agent, immutable object/cloud storage adapters, prompt CRUD, a generic CDC connector and non-PostgreSQL database connectors are not. |
...
| Night-All saved records | Repository implementation, default paused: 13 `saved_records` LIST leaves have independent fixed sources, provider-neutral datasets/platforms, checkpoints and failure states. Credential-free migration 066 seeds unapproved mappings; activation requires exact source-schema probes, valid unique `(last_seen_at,id)` leaf indexes and an explicit writer/delete/commit-order attestation. `record_key` is the dataset-scoped canonical identity, large JSON/raw evidence stays restricted, and a fixed cleaner owns published-time and publisher/collector catalog classification. Public stored/canonical search enforces candidate-only independently in PostgreSQL and content-v6 Elasticsearch for every `data_center_saved_records_*` branch; mixed queries preserve other platforms, while the legacy live-compatible search rejects these Hub-stored platforms. No public grant is seeded, and non-news or content-empty records remain internal pending review. |
...
| Xiaohongshu note product | Implemented: the first-class Hub-owned platform-shaped `POST /api/v1/xiaohongshu/app/get_note_info` accepts a JSON note link without placing temporary link parameters in the request target; its GET form retains `share_text`/`note_id` compatibility, and provider-neutral `POST /api/v1/data/post` remains available. All forms share note identity, snapshot and dispatch suppression, while exact idempotency also binds delivery mode. They return stable note content, author, engagement, tags and media, use exact cache/fallback evidence, and enqueue canonical ingestion after a successful live response. Eligible Xiaohongshu `/api/v1/data/search` and legacy `/api/v1/night-all/search/raw` requests can use the Hub-owned TikHub route while preserving their public contracts; 60-boundary previews receive bounded detail enrichment and explicit completeness evidence. The ink-scroll Admin product and browser-local history are implemented. Durable object storage and durable cross-device history are not; current images use the authenticated bounded media relay. |
| Nationwide province public opinion | Repository implementation, default paused: the fixed PostgreSQL source, province hot/latest/detail/coverage boundary, append-only raw source revisions and a source-revision-anchored rule/Agent analysis pipeline are defined. Additive P1 contracts provide a stable 34-province catalog and a bounded nationwide-or-province region feed for `CN` or one exact catalog code. The feed is fixed to `visibility=all_ingested`, `sort=latest` and required `from`/`to`, and requires both the `public_opinion` platform grant and non-default `public_opinion.all_ingested.read` capability. Its `canonical_current_safe` corpus includes unclassified, unscored and rejected current records, while excluding raw rows, history, deleted records and records without a revision-fenced current publication state. Existing province/coverage/detail/search APIs are unchanged; city catalog/feed is P2. Night-All migration `042_monitor_strategy_results_hub_watermark.sql` provides the ordered writer contract; additive migration `043_monitor_strategy_result_source_stage.sql` lets the existing result table carry `formal` and gated `candidate` source rows without creating a second product table. Hub migration 035 owns revision-fenced publication, quality and geography state; its bounded typed ES publication fields were introduced in content-v5 and remain in current content-v6. Raw assertions and provider evidence remain private. Rollout must install and verify Hub 035 plus formal-only serving gates first, then apply Night-All 042/043 to its database and upgrade every Night-All reader while the writer gate stays off; only after old readers have drained may operators enable the candidate writer and, separately, the paused Hub analysis pipeline. No environment rollout, upstream connection, data import, provider setup or consumer grant is implied. |
| Search/retrieval | Canonical projection outbox, projector, unified cross-platform stored search, strict Chinese relevance, PostgreSQL degradation paths, Admin semantic search and a guarded Admin-plane reindex operation are implemented. The repository includes versioned allowlisted profiles and the content-v6 mapping; each deployed environment remains gated on its strict blue/green index validation. Elasticsearch remains rebuildable and is not required for canonical/history availability. |
...
| Plans, quota and billing | Implemented now: versioned plans, immutable customer price books, tenant PPM multipliers, disabled/shadow/enforced rollout modes, request-time customer charge snapshots, a tenant prepaid wallet with append-only ledger, admin-only idempotent manual credit, audited single-request unknown-hold reconciliation, effective plan/consumer/key quota enforcement, tenant-scoped final-rate/balance/call views, and an admin-only provider procurement view. Successful logical Hub deliveries are the customer billing unit; internal provider fan-out and idempotent replay do not add customer charges. Existing price-book-free plans remain unbilled. For explicitly funded/billed requests, provider monthly procurement/subsidy thresholds are warning-only, while provider rate/concurrency/circuit/contract protections and customer quotas remain enforced. `launch-1m` keeps its 1,000,000/month total plus 100 RPS and page-size 100; platform and Key windows remain independent. Pending: online payment, self-service purchase/assignment, subscription lifecycle, invoice engine, reconciliation approval workflow, BI and Data Agent monetization. |
...
- The repository declares content v6. v4's bounded raw/HanLP/CJK/prefix views
  remain the existing profile minimum, and v5 remains the historical milestone
  that introduced revision-fenced typed public-opinion publication, quality,
  geography and effective-time fields. v6 preserves those fields and adds the
  typed crawler publication eligibility used to enforce candidate-only public
  canonical search. An environment whose read alias still points to v5 or
  earlier uses the PostgreSQL visibility predicate for affected first pages;
  old ES PITs return `503 search_cursor_unavailable`. Only a strict PG-to-ES
  full rebuild and alias cutover activates content-v6. Crawler scopes add the
  versioned visibility contract to idempotency fingerprints and signed cursor
  bindings; scopes containing neither crawler nor public-opinion keep their
  previous fingerprint and cursor semantics.
...
- The authoritative lifecycle and guardrails are documented in
  ...
  [Search and observability stack](operations/search-and-observability-stack.md#42-content-v6-与搜索-profile-变更手册),
...
4. [Transitional Night-All integration](architecture/night-all-integration.md)
...
17. [Local development](operations/local-development.md)
18. [Internal K8s deployment](operations/internal-k8s-deployment.md)
...
22. [Night-All saved-records ingestion](operations/night-all-saved-records-ingestion.md)
23. [Virtual supermarket data product and public contract](product/virtual-supermarket.md)
24. [Ecommerce data treasure-box product and interaction design](product/ecommerce-data-treasure-box.md)
25. [JustOne capability map and Hub adoption plan](integrations/justone-capability-map.md)
26. [Xiaohongshu direct TikHub migration boundary](integrations/xiaohongshu-direct-tikhub-migration.md)
27. [External data platform operations](operations/external-data-platforms.md)
28. [Hub-owned static media data plane](adr/0014-hub-owned-static-media-data-plane.md)
29. [Nationwide province public-opinion source](architecture/province-public-opinion-source.md)
30. [Nationwide public-opinion sourcing and geography evidence](architecture/nationwide-public-opinion-sourcing.md)
31. [Nationwide province public-opinion operations](operations/province-public-opinion-ingestion.md)
32. [Public-opinion volume, quality and archive troubleshooting](operations/public-opinion-volume-quality-archive.md)
33. [Backup and restore](operations/backup-restore.md)
34. [Observability and SLO](operations/observability-slo.md)
35. [BI and Data Agent evolution](architecture/bi-and-data-agent-evolution.md)
36. [Agent Market advanced-search dry run](architecture/agent-market-advanced-search.md)
37. [Agent Studio governed lifecycle](architecture/agent-studio-governed-lifecycle.md)
38. [Agent Studio LangGraph runtime and Hub data-node design](architecture/agent-studio-langgraph-data-orchestration.md)
39. [ADR-0012: Hub-native Agent Studio](adr/0012-hub-native-agent-studio.md)
40. [Historical Agent Studio build-vs-buy evaluation (superseded)](architecture/agent-studio-platform-boundaries-and-build-vs-buy.md)
41. [Agent provider settings](operations/agent-provider-settings.md)
42. [Open capabilities, file rules and bounded classification cost](adr/0008-open-capabilities-file-rules-and-classification.md)
