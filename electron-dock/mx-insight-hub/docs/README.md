...
| Ecommerce data treasure box | Implemented as an Admin-token-only JustOne product-search workbench above the provider-neutral `POST /api/v1/data/ecommerce/products/search` contract. The original “小聚” mascot exposes idle/searching/presenting states, keyboard-accessible product spheres, a browser-local safe demo that sends no Public search, a live mode that keeps a Hub consumer key only in component memory, and truthful live/cache/fallback/replay evidence. Split deployments deliver the non-secret `MX_INSIGHT_PUBLIC_URL` origin through the authenticated Admin session; ambiguous retries retain the exact body and idempotency key, while changing the Hub key invalidates local replay. Admin results use neutral icons instead of auto-loading upstream images. Public `/docs/ecommerce-treasure-box` remains provider-neutral; the Admin capability atlas and internal JustOne map distinguish five verified marketplaces from provider-catalog candidates. |
