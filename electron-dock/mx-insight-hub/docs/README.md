...
| Ecommerce data treasure box | Implemented as an Admin-token-only JustOne product-search workbench above the provider-neutral `POST /api/v1/data/ecommerce/products/search` contract. The original “小聚” mascot exposes idle/searching/presenting states, keyboard-accessible product spheres, a browser-local safe demo that sends no Public search, a live mode that keeps the ordinary Hub Public API key only in component memory, and truthful live/cache/fallback/replay evidence. External ecommerce search and media accept that same ordinary key only when it is `mih_live_`; no product-specific second key exists. With a valid legacy Test key, capabilities reports granted ecommerce as `ready=false` and search/media return `403 test_key_not_supported` before usage or external I/O. A historical ambiguous Test record remains locked for operator reconciliation and is never sent by the page. Split deployments deliver the non-secret `MX_INSIGHT_PUBLIC_URL` origin through the authenticated Admin session; Live ambiguous retries retain the exact body and `Idempotency-Key`. Changing the Hub key may discard resolved convenience state, but never clears an ambiguous lock; a mismatch requires the original Live secret or operator reconciliation. Admin results load retained images only through the authenticated bounded media relay. Public `/docs/ecommerce-treasure-box` remains provider-neutral; the Admin capability atlas and internal JustOne map distinguish five verified marketplaces from provider-catalog candidates. |
