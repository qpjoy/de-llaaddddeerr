...
| Night-All compatibility and direct migration | Implemented for the provider-neutral `/api/v1/data/search` facade and the three explicit `/api/v1/night-all/search/{raw,crawl,user-info}` migration routes. Eligible Xiaohongshu raw and narrow single-user crawl/user-info shapes can use separately gated Hub-native TikHub workflows; historical `mxnc1`, batch/channel/non-post and other unsupported shapes remain on Night-All. Compatibility delivery does not desensitize, filter or truncate business data; only pagination controls are governed. Historical lineage retains complete parsed JSON and legacy raw strings, while Hub-native provider calls add restricted, bounded exact-byte archives. Request credentials stay out of responses, ordinary UI and logs. Repository presence is not proof that a rollout gate is active. |
