...
- Provider connectivity evidence is scoped to that Provider's effective egress route. Creating or editing an unbound Proxy catalog entry must not invalidate a direct Provider test; changing the Provider-specific or Hub-global route must invalidate it.
...
- Records referenced by Agent control objects are protected by the server transaction, not only by disabled UI actions: a Provider must leave every LLM Sequence before deletion, a Proxy endpoint must leave every Proxy Sequence, and a Proxy Sequence must be unbound from Hub-global and every Provider.
