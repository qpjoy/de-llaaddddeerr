  ...
  paths: {
    ...
    '/data/aggregate/preview': {
      post: {
        ...
        description: 'Uses the same body and continuation cursor as aggregate search. Read-only estimate: no data acquisition, usage reservation or wallet hold. Returns the current assigned plan version, per-source meter, one-page price and observed readiness, total estimatedMinor and estimateOnly=true. Unknown runtime readiness is not_checked; unavailable sources are explicitly marked. Explicit plan rates (including zero) take precedence; other metered operations use the tenant default request price, initially zero. Tenant default prices are final prices and do not inherit plan multipliers. Execution rechecks authorization, price and runtime; this estimate is neither a locked quote nor a spending cap. Live parentChargeMinor is zero; children keep their own contract rates. Never returns procurement prices or supplier identity.',
    ...
    '/data/aggregate/search': {
      post: {
        ...
        requestBody: aggregateSearchRequestBody,
        responses: { 200: { description: 'JSON returns the committed response. SSE events: search.started {requestId,totalSources,execution}; source.started {id,platform,label}; source.completed {source,items}; search.completed is the full committed JSON envelope plus replay; search.error is a terminal failure after headers. Before streaming, errors retain their normal HTTP status/JSON. Heartbeats every 10s. Only search.completed supplies a usable parent cursor. Disconnect does not cancel paid work. A same-key in-progress retry returns 409; completed retry delivers the stored response with replay=true. execution.concurrency=3, dispatchBudgetMs=120000: stop new source dispatches, then drain calls under connector deadlines; hardResponseDeadlineMs=null. not_started/aggregate_dispatch_deadline means no call was started and will not be automatically retried. Items are a bounded page, never a total. Other source statuses: ok, empty, partial, unsupported, not_authorized, unavailable, unknown.',
          content: { 'application/json': { schema: { type: 'object' } }, 'text/event-stream': { schema: { type: 'string' } } } }, ...publicErrors },
      },
    },
    ...
    schemas: {
      ...
      NightAllLegacyRequest: {
        ...
        properties: {
          ...
          cursor: { type: 'string', minLength: 1, maxLength: 8192, description: 'Return data.page.nextCursor unchanged here for every compatibility pagination mode. Only an opaque Hub cursor returned by the preceding page is accepted. Historical raw provider cursors fail closed with invalid_cursor; restart without cursor and use a new Idempotency-Key.' },
          ...
          params: { type: 'object', additionalProperties: true, description: 'Safe non-continuation platform values are accepted on the first page. All next pages can use the top-level cursor from data.page.nextCursor. The Hub-issued nextParams object (currently {cursor: mxnc1...}) remains a legacy alias; if both cursor entries are sent they must match. Raw provider continuation values fail closed. Rejected keys and workload overrides are listed by x-mx-rejected-params.' },
        },
      },
      StoredSearchRequest: {
        type: 'object',
...
printf '%s\n' "$REPORT" | jq '{id:.data.id,status:.data.status,progress:.data.progress,requestId}'</code></pre>
  ...
  | jq '{status:.data.status,phase:.data.phase,progress:.data.progress,summary:.data.result.executiveSummary,coverage:.data.result.coverage}'</code></pre>
    ...
    <h3>逐来源流式返回与等待边界</h3>
    <p>默认仍返回 JSON。交互式应用可在相同 POST 请求设置 <code>Accept: text/event-stream</code>，使用支持 Authorization 与 JSON body 的 fetch；无需另建任务，也不更改原接口或幂等标识。SSE 的 <code>search.started</code> 给出请求 ID 和本批来源数，<code>source.started</code> 标记正在调用，<code>source.completed</code> 交付该来源的安全字段与结果；最终 <code>search.completed</code> 才是已提交的完整响应和分页游标。</p>
    <p>每 10 秒发送注释心跳；服务端并发为 3，120 秒后停止派发新来源，状态为 <code>not_started / aggregate_dispatch_deadline</code>。已发出的调用按各接口超时收尾，保留结算证据，因此这是调度截止，<strong>不是整轮响应的硬超时</strong>。部分成功、无结果、未知结果与未开始分别报告，不以返回条数推断完整覆盖。</p>
    <p>开始流式输出前，参数或权限错误仍使用原 HTTP 状态和 JSON 错误；输出后失败以 <code>search.error</code> 终止。只有 HTTP 200 不代表整轮完成。连接中断不取消已派发的调用；不要自动更换标识重采。用原 body 与 Idempotency-Key 重试，进行中返回 409，提交后只回放最终响应；暂不支持 Last-Event-ID 或从某个事件断点续传。反向代理应关闭此响应的缓冲，读取超时应覆盖完整调用周期。</p>
...
curl -sS "$HUB_URL/api/v1/data/canonical/items/$ANCHOR_ID/context?before=10&amp;after=10" \\
  ...
  -d '{"platform":"twitter","username":"openai"}'</code></pre>
    <div class="notice"><strong>crawl 采集工作预算与费用独立。</strong>单页有效条数（count → pageSize → limit，默认 20）仍受平台/Key 策略及上游最多 100 条限制。总工作量 = 身份数 × 每身份条数 × activityTypes 数（省略时为 1），由管理员在“开放能力 → 调用身份 → 平台配置”的 <code>maxCrawlWork</code> 中设置，范围 1–5000，默认 100；旧平台策略保留其不超过 100 的原限制。同类身份别名中完全相同的值只计一次，不推断 username 与 userId 是否为同一人。最多 50 个身份的独立限制仍生效。此预算是单请求处理量，不是累计配额、采购价或客户单价。</div>
    <p>例如两个账号各取 100 条、一个活动类型，工作量为 200：预算 100 时返回 <code>400 work_budget_exceeded</code>，管理员调至 200 或拆成两次单账号请求即可通过该预检（仍需满足其他授权与配额）。单账号、一个活动类型、count=100 本来就不超过 100；仅凭 target_count=100 不能判定完整请求工作量。免费价格不能跳过预检，公开请求也不能自报 maxCrawlWork。</p>
    <pre><code>{"error":{"code":"work_budget_exceeded","message":"Crawl work must not exceed 100","details":{"stage":"admission","upstreamDispatched":false,"retryable":false,"identityCount":2,"pageSize":100,"activityTypeCount":1,"requestedWork":200,"allowedWork":100}}}</code></pre>
    <p>上述采集总预算拒绝发生在预留计费与调用上游之前。下游应提示调整预算或拆分请求，不应原参数连续重试；身份数超限也是同一错误码，应按错误 message 区分。修改请求后使用新的 Idempotency-Key。提高预算不承诺上游具备相应吞吐或一定返回目标数量。</p>
    <p>历史 Night-All 路径不会向调用方暴露 provider continuation：首屏返回的 cursor、composite/offset params 或页码会被替换成 consumer、operation、platform、查询范围和页码绑定的加密 <code>mxnc1</code> cursor。所有分页模式统一读取 <code>data.page.nextCursor</code> 并原样回传至请求的顶层 <code>cursor</code>，无需按平台拼装参数。原有 composite/offset 的 <code>nextParams</code> 和 mode 保留兼容；若同时回传两个游标入口，其值必须一致。下一页使用新的 <code>Idempotency-Key</code>；第 15 页强制结束。升级前保存的裸 provider cursor/continuation params 返回 <code>400 invalid_cursor</code>，需去掉 cursor、换新 Key 从首页重启。</p>
