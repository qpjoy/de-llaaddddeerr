...
export function TenantServiceAccess({token,tenants,platforms,capabilities}) {
  ...
  return <section className="qp-panel">
    ...
    {form ? <>
      <div className="mih-xhs-detail-actions"><button className="qp-button qp-button--outline" disabled={busy} onClick={preset}>勾选小红书笔记所需权限</button><button className="qp-button qp-button--outline" disabled={busy} onClick={()=>setForm(f=>({...f,platforms:[],capabilities:[]}))}>清空本次勾选</button></div>
      ...
      <p>取消已开通范围会收回该租户现有调用者的对应授权；旧 Key 随即受限。增加授权不会扩大旧 Key 的签发快照，需要重新签发 Key。原先由管理员单独开通且不在本租户配置中的范围保持原状。</p>
      ...
      {saved ? <p role="status">已保存。租户刷新开放能力或重新打开签发 Key 窗口即可看到授权。</p> : null}
    </> : null}
  </section>
}
