...
export function requestSnippet({ format, url, body, credential, idempotencyKey, method = 'POST' }) {
  ...
  if (format === 'powershell') return [
    ...
    `Invoke-RestMethod -Method ${method === 'POST' ? 'Post' : method} -Uri ${psQuote(url)} -Headers $headers${hasBody ? ` -Body ([System.Text.Encoding]::UTF8.GetBytes(${psQuote(json)}))` : ''}`,
  ].join('\n')
  const call = `const response = await fetch(${JSON.stringify(url)}, ${JSON.stringify({ method, headers, ...(hasBody ? { body: json } : {}) }, null, 2)});\nconsole.log(response.status, await response.text());`
