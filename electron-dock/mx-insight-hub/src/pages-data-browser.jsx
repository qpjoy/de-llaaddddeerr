...
const views = [['accounts', '账号大盘'], ['contents', '内容大盘'], ['hotspots', '热点线索'], ['advanced', '高级搜索'], ['diagnostics', '请求诊断']]
...
export function DataBrowserPage({ token, onUnauthorized }) {
  ...
  const load = useCallback(async () => ({ ...(advanced || diagnostics || filters.account && accountTab !== 'contents' ? { items: [] } : await adminApi.dataBrowser(token, filters)), filterKey }), [token, filters, filterKey, accountTab, advanced, diagnostics])
  const state = useRemoteData(load, onUnauthorized)
  ...
  const total = useBrowserTotal(token, filters, !advanced && !diagnostics && readyScope === scopeKey, onUnauthorized)
  ...
  const navigate = (view, values = {}) => { setDiagnostics(view === 'diagnostics'); if(view==='diagnostics'){setAdvanced(false);return}if(view==='advanced'){setAdvanced(true);return}setAdvanced(false); const search = { ...emptySearch, ...values }; setDraft(search); patch({ ...search, view, account: '', ...values }); setAccountRow(null) }
