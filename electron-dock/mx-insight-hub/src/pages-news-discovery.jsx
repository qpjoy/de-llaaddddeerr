...
export function NewsDiscoveryPage({ session }) {
  ...
  return <div className="mih-news">
    ...
    <form className="qp-panel mih-news-filters" onSubmit={event => { event.preventDefault(); void search() }}>
      ...
      <div className="mih-news-actions"><button className="qp-button qp-button--primary" disabled={!key || busy}><MagnifyingGlass size={17} />{busy ? '正在查询…' : '查询新闻'}</button>
        <button type="button" className="qp-button qp-button--outline" disabled={!applied || busy || !key} onClick={() => void statistics()}>查看来源统计</button></div>
