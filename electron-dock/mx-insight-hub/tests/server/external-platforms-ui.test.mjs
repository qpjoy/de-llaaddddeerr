...
test('external-platform page exposes JustOne, TikHub and Night-All while keeping admin-token session gating', async () => {
  ...
  assert.match(pageSource, /const SUPPORTED_PROVIDERS = new Set\(\['justone', 'tikhub', 'night-all', 'ipsearch'\]\)/u)
  ...
  assert.match(pageSource, /hasAggregateSummary \? normalizeCost\(\{ billing: payload\.summary \}\)/u)
