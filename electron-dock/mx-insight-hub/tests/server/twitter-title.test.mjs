...
for (const platform of ['twitter', 'facebook']) {
  ...
  test(`${platform} data-search normalization clears legacy titles without mutating source evidence`, () => {
    for (const title of [undefined, '', generatedTitle, body.slice(0, 80)]) {
      ...
      const { records, skipped } = normalizeSearchPayload({ data: { items: [item] } }, platform)
      ...
      assert.equal(records[0].contentType, platform === 'twitter' ? 'tweet' : null)
      assert.deepEqual(records[0].rawItem, original)
      assert.deepEqual(item, original)
    }
  })

  test(`${platform} legacy content leaves title empty while profiles and archived envelopes remain intact`, () => {
    ...
    for (const operation of ['raw', 'crawl', 'user-info']) {
      const { records, skipped } = normalizeNightAllLegacyPayload(payload, platform, operation)
      ...
      // Account labels are independent from a content title.
  ...
  test(`${platform} media-only posts remain ingestible without a fabricated title`, () => {
    const payload = { data: { raw_info: '[]', raw_data: JSON.stringify([
      { content_id: externalId, image_urls: '["https://example.test/image.jpg"]' }
    ]) } }
    const { records, skipped } = normalizeNightAllLegacyPayload(payload, platform, 'raw')
    assert.equal(skipped, 0)
    assert.equal(records.length, 1)
    assert.equal(records[0].title, null)
  ...
  test(`${platform} title correction changes the old digest once and stays stable on subsequent sightings`, () => {
    const item = { externalId, title: generatedTitle, text: body }
    const normalize = value => normalizeSearchPayload({ data: { items: [value] } }, platform).records[0]
    const first = normalize(item)
    assert.notEqual(first.payloadSha256, sha256(canonicalJson(item)))
    if (platform === 'twitter') {
      assert.equal(first.payloadSha256, sha256(canonicalJson({ parserVersion: 'mxih-normalizer.v2', content: item })))
    }
