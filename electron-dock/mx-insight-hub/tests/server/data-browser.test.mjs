...
test('browser endpoint requires Admin Token and is absent on public listener', async () => {
  for (const listenerMode of ['combined', 'public']) {
    ...
    try {
      ...
      const statsDenied = await fetch(`${url}/statistics`)
      assert.ok([401, 404].includes(statsDenied.status))
      const stats = await fetch(`${url}/statistics?view=accounts`, { headers: { 'x-mx-insight-admin-token': 'browser-test' } })
      assert.equal(stats.status, listenerMode === 'public' ? 404 : 200)
