...
function run(token, ids = [], { providerLedgerMissing = false } = {}) {
  ...
  const stub = `const pg = { Client: class {
    ...
    async query(sql) {
      ...
      if (${providerLedgerMissing} && /FROM external_platform.provider_calls/.test(sql)) throw Object.assign(new Error('private-db-detail'), { code: '42P01' });
      if (/FROM external_platform.provider_calls|FROM billing.customer_charges/.test(sql)) return { rows: [] };
  ...
  const result = spawnSync(process.execPath, ['--input-type=module', '-', ...ids], {
    input: code, encoding: 'utf8',
    env: { ...process.env, DATABASE_URL: 'synthetic-not-connected', MX_INSIGHT_API_KEY_PEPPER: secret },
  })
  assert.equal(result.status, 0, result.stderr)
  for (const sensitive of [secret, token, 'private-search-context', 'private-backtrace', 'private-db-detail']) {
...
for (const complete of [false, true]) {
  test(`readonly cursor diagnosis distinguishes ${complete ? 'complete' : 'old single-value'} tokens`, () => {
    ...
    assert.deepEqual(report.requests[0].cursorState, {
      status: 'authenticated', platformMatchesRequest: true, operationIsRaw: true, operation: 'raw',
...
test('batch diagnosis reports requested IDs that have no usage record', () => {
  const ids = ['37bf1066-508d-47c9-9c54-2d572e03a0a5', '98a5c405-dbe4-4ccc-bf0a-5e6907ca7320']
  const report = run('mxnc1.invalid-token', ids)
  assert.deepEqual(report.requestIds, ids)
  assert.deepEqual(report.missingRequestIds, [ids[1]])
  assert.deepEqual(report.providerCalls, [])
  assert.deepEqual(report.customerCharges, [])
})

test('an unavailable ledger is an explicit error and does not suppress other sections', () => {
  const report = run('mxnc1.invalid-token', [], { providerLedgerMissing: true })
  assert.deepEqual(report.providerCallsError, { code: '42P01' })
  assert.equal(report.providerCalls, undefined)
  assert.deepEqual(report.customerCharges, [])
