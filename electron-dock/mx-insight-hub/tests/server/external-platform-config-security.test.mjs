...
test('runtime constructs the paid adapter only after explicit contract verification', async () => {
  ...
  try {
    ...
    assert.equal(awaiting.externalPlatformGateway.capabilities().ready, false)
  } finally {
  ...
  try {
    ...
    assert.equal(verified.externalPlatformGateway.capabilities().ready, true)
...
test('bad optional-provider configuration keeps unrelated runtime services available', async () => {
  ...
  try {
    ...
    const publicCapabilities = runtime.externalPlatformGateway.capabilities()
