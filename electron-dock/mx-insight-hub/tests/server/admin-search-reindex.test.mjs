...
test('Admin search reindex is durable single-flight work with progress and audit evidence', async () => {
  ...
  assert.equal(first.preflight.targetIndexSchema, 'content-v6')
  await started
...
test('Admin reindex HTTP routes are admin-token-only and accept no execution parameters', async () => {
  ...
  const status = {
    preflight: {
      ...
      sourceIndexSchema: 'content-v3', targetIndexSchema: 'content-v6',
    },
    operation: null,
  }
  const searchReindex = {
...
test('a reachable but degraded cluster warns instead of vetoing its own replacement', async () => {
  ...
  assert.equal(preflight.targetIndexSchema, 'content-v6')
})
...
test('a target shard the cluster refuses to place blocks with the deciders that refused it', async () => {
  const { pool } = fakeDatabase()
  const search = healthySearch(pool)
  search.indexSet = { schemaVersion: 6, currentIndex: 'mx-insight-hub-content-v6-current' }
  search.client.clusterHealth = async () => ({
  ...
  search.client.allocationExplain = async ({ index, shard, primary }) => {
    assert.equal(index, 'mx-insight-hub-content-v6-current')
  ...
  assert.match(blocker.message, /mx-insight-hub-content-v6-current shard 0/)
...
test('preflight reads tokenizer provenance from the actual serving A/B generation', async () => {
  ...
  search.indexSet = {
    ...
    writeAlias: 'mx-insight-hub-content-v6',
    currentIndex: 'mx-insight-hub-content-v6-current',
  }
  const serving = 'mx-insight-hub-content-v6-rebuild'
