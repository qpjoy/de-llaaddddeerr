...
export async function hdoRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/api/v1/hdo/anonymous/bootstrap', async (req, reply) => {
    ...
    return {
      ...
      device,
      manifest,
      domains: domainBindingsFromDnsRecordsAndServices(manifest.dnsRecords, manifest.services)
    };
  });
  ...
  app.get('/api/v1/hdo/admin/dns/hosts', { preHandler: requireHdoAdminOrRunner }, async (req, reply) => {
    const content = await renderHdoDnsHosts();
    reply.type('text/plain; charset=utf-8');
    return content;
  });
  ...
  app.get('/api/v1/hdo/admin/dns', adminOnly, async () => hdoStore.listDnsRecords());

  app.post('/api/v1/hdo/admin/dns', adminOnly, async (req, reply) => {
    const body = (req.body ?? {}) as Record<string, unknown>;
    const domain = normalizeHdoDnsDomain(asRequiredString(body.domain));
    const targetHost = normalizeHdoDnsTargetHost(asRequiredString(body.targetHost));
    if (!domain || !targetHost) {
      reply.code(400);
      return { error: 'domain and valid targetHost required' };
    }
    const row = await hdoStore.upsertDnsRecord({
      id: asOptionalString(body.id) ?? undefined,
      domain,
      targetHost,
      enabled: asOptionalBoolean(body.enabled) ?? true,
    ...
    await auditStore.insert({
      actorUserId: req.currentUser!.id,
      actorIp: req.ip,
      action: 'hdo.dns.upsert',
      targetKind: 'hdo_dns_record',
      targetId: row.id,
      meta: { domain: row.domain, targetHost: row.targetHost, enabled: row.enabled }
    });
    return row;
  });
...
function buildHdoDeploymentInvocation(
  ...
  if (kind === 'sync-dns') {
    return { args: ['sync-dns', '--server-url', serverUrl], serverUrl };
  }
  if (kind === 'status') {
...
async function renderHdoDnsHosts(): Promise<string> {
  const [dnsRecords, services] = await Promise.all([
    hdoStore.listDnsRecords(),
    hdoStore.listServices()
  ]);
  const lines = [
    '# Generated by electron-server. Manual edits will be replaced.',
    '# Explicit HDO DNS records take priority over service domains.'
  ];
  const seen = new Set<string>();
  for (const record of dnsRecords) {
    if (!record.enabled) continue;
    const domain = normalizeHdoDnsDomain(record.domain);
    const targetHost = normalizeHdoDnsTargetHost(record.targetHost);
    if (!domain || !targetHost || seen.has(domain)) continue;
    seen.add(domain);
    lines.push(`${targetHost} ${domain}`);
  }
  for (const service of services) {
    if (!service.enabled) continue;
    const targetHost = normalizeHdoDnsTargetHost(service.targetHost);
    if (!targetHost) continue;
    for (const value of service.domains) {
      const domain = normalizeHdoDnsDomain(value);
...
async function buildManifest(device: HdoDeviceRow, options: HdoManifestOptions = {}) {
  ...
  const visibleServices = meshAccess.active
    ...
    : [];
  const visibleDnsRecords = meshAccess.active
    ? dnsRecords.filter(
        (row) =>
          row.enabled &&
          (!options.anonymous || isAnonymousVisibleMetadata(row.metadata, meshAccess.groupIds)) &&
          isVisibleForMesh(row.metadata, meshAccess.groupIds)
      )
    : [];
...
function renderMihomoYaml(manifest: Awaited<ReturnType<typeof buildManifest>>): string {
  ...
  for (const record of manifest.dnsRecords) {
    rules.add(`DOMAIN-SUFFIX,${record.domain},HDO`);
    if (isIpv4(record.targetHost)) {
      rules.add(`IP-CIDR,${record.targetHost}/32,HDO,no-resolve`);
    }
  }
...
function domainBindingsFromDnsRecordsAndServices(
  ...
  for (const service of services) {
    for (const domain of service.domains) {
      const normalized = normalizeHdoDnsDomain(domain);
...
function normalizeHdoDnsDomain(value: string | null): string | null {
  const text = asOptionalString(value)?.toLowerCase().replace(/^\.+|\.+$/g, '');
...
function normalizeHdoDnsTargetHost(value: string | null): string | null {
  const host = normalizeOverlayIp(asOptionalString(value));
  return host && isIpv4(host) ? host : null;
}
