...
const { chmodSync, mkdirSync, mkdtempSync, readFileSync, rmSync, statSync, writeFileSync } = require('node:fs');
...
function createManagedServerFixture(home, peerMesh = false) {
  const pki = join(home, 'pki');
  const ccd = join(home, 'ccd');
  const clients = join(home, 'clients');
  mkdirSync(pki, { recursive: true });
  mkdirSync(ccd, { recursive: true });
  mkdirSync(clients, { recursive: true });

  const config = join(home, 'server.conf');
  const env = join(home, 'server.env');
  const restartLog = join(home, 'restart.log');
  ...
  const originalEnv = [
    '# Generated by qp-tunnel-cli open install. Do not edit by hand.',
    installedEnv.replace(
      'QP_OPEN_CLIENT_TO_CLIENT=false',
      `QP_OPEN_CLIENT_TO_CLIENT=${peerMesh}`,
    ),
    "QP_OPEN_FUTURE_FIELD='preserve-me'",
    '',
  ].join('\n');

  writeFileSync(config, originalConfig, { mode: 0o600 });
  writeFileSync(env, originalEnv, { mode: 0o600 });
  writeFileSync(join(pki, 'ca.key'), 'existing-ca-key\n');
  writeFileSync(join(ccd, 'internal-01'), 'ifconfig-push 100.127.0.10 255.255.255.0\n');
  writeFileSync(join(clients, 'internal-01.ovpn'), 'existing-issued-profile\n');
  writeFileSync(join(home, 'state.json'), '{"existing":true}\n');

  return { config, env, restartLog, originalConfig, originalEnv, pki, ccd, clients };
}
...
test('reconfigure is dispatched as an instance-aware server command', () => {
  const result = spawnSync(
    'bash',
    [packagedScript, 'reconfigure', '--instance', 'jp01', '--help'],
    { encoding: 'utf8' },
  );
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /open reconfigure --client-to-client/);
...
test('the server configuration pushes nothing to spokes', () => {
  ...
  assert.doesNotMatch(generated, /^disable-dco$/m);
  assert.doesNotMatch(generated, /^ignore-unknown-option disable-dco$/m);
});

test('client-to-client uses an ordered DCO compatibility block when explicitly requested', () => {
  ...
  const guardAt = lines.indexOf('ignore-unknown-option disable-dco');
  const disableAt = lines.indexOf('disable-dco');
  const meshAt = lines.indexOf('client-to-client');

  assert.notEqual(guardAt, -1);
  assert.notEqual(disableAt, -1);
  assert.notEqual(meshAt, -1);
  assert.ok(guardAt < disableAt, 'the compatibility guard must precede disable-dco');
  assert.ok(disableAt < meshAt, 'DCO must be disabled before enabling the userspace mesh');
  assert.match(generated, /^client-to-client$/m);
});

test('reconfigure enables peer mesh transactionally and is idempotent', () => {
  const home = join(testRoot, 'srv-reconfigure-success');
  ...
  const result = runLibrary(
    ...
    String.raw`
      ...
      restart_running_server() { echo restart >> "${fixture.restartLog}"; }
      restart() { return 97; }
      build_pki() { return 97; }
      write_server_config() { return 97; }
      save_env() { return 97; }
      apply_firewall() { return 97; }
      install_firewall_unit() { return 97; }
      write_state() { return 97; }
      reconfigure_server --client-to-client
      reconfigure_server --client-to-client
    `,
  );
  assertOk(result);

  const config = readFileSync(fixture.config, 'utf8');
  const env = readFileSync(fixture.env, 'utf8');
  const lines = config.trim().split('\n');
  assert.equal(lines.filter((line) => line === 'ignore-unknown-option disable-dco').length, 1);
  assert.equal(lines.filter((line) => line === 'disable-dco').length, 1);
  assert.equal(lines.filter((line) => line === 'client-to-client').length, 1);
  assert.equal(lines.filter((line) => line === '# BEGIN qp-tunnel-cli peer-mesh').length, 1);
  assert.equal(lines.filter((line) => line === '# END qp-tunnel-cli peer-mesh').length, 1);
  assert.ok(lines.indexOf('ignore-unknown-option disable-dco') < lines.indexOf('disable-dco'));
  assert.ok(lines.indexOf('disable-dco') < lines.indexOf('client-to-client'));
  assert.match(config, /^port 443$/m);
  assert.match(config, /^# operator note must survive$/m);
  assert.match(env, /^QP_OPEN_SUBNET=100\.127\.0\.0\/24$/m);
  assert.match(env, /^QP_OPEN_CLIENT_TO_CLIENT='true'$/m);
  assert.match(env, /^QP_OPEN_FUTURE_FIELD='preserve-me'$/m);
  assert.equal(readFileSync(fixture.restartLog, 'utf8'), 'restart\n');
  assert.equal(statSync(fixture.config).mode & 0o777, 0o600);
  assert.equal(statSync(fixture.env).mode & 0o777, 0o600);

  // Reconfiguration must never regenerate or rewrite identity, addressing,
  // profiles, state, firewall, or any client-side material.
  assert.equal(readFileSync(join(fixture.pki, 'ca.key'), 'utf8'), 'existing-ca-key\n');
  assert.equal(
    readFileSync(join(fixture.ccd, 'internal-01'), 'utf8'),
    'ifconfig-push 100.127.0.10 255.255.255.0\n',
  );
  assert.equal(
    readFileSync(join(fixture.clients, 'internal-01.ovpn'), 'utf8'),
    'existing-issued-profile\n',
  );
  assert.equal(readFileSync(join(home, 'state.json'), 'utf8'), '{"existing":true}\n');
  assert.match(result.stdout, /no files changed and no restart was needed/);
});

test('reconfigure leaves an inactive server inactive', () => {
  const home = join(testRoot, 'srv-reconfigure-inactive');
  const fixture = createManagedServerFixture(home);
  writeFileSync(fixture.restartLog, '');
  const result = runLibrary(
    ['status'],
    String.raw`
      QP_OPENS_HOME="${home}"
      ...
      restart_running_server() { echo unexpected-restart >> "${fixture.restartLog}"; }
      reconfigure_server --client-to-client
    `,
  );
  assertOk(result);
  assert.equal(readFileSync(fixture.restartLog, 'utf8'), '');
  assert.match(result.stdout, /server was inactive and was left inactive/);
});

test('reconfigure restores the exact previous files when the new config fails to start', () => {
  const home = join(testRoot, 'srv-reconfigure-rollback');
  const fixture = createManagedServerFixture(home);
  writeFileSync(fixture.restartLog, '');
  const result = runLibrary(
    ['status'],
    String.raw`
      ...
      restart_running_server() {
        echo restart >> "${fixture.restartLog}"
        [[ "$(wc -l < "${fixture.restartLog}")" -ge 2 ]]
      }
      reconfigure_server --client-to-client
    `,
  );
  ...
  assert.match(result.stderr, /previous configuration was restored and is running/);
...
test('reconfigure EXIT guard restores both files when the second atomic replace fails', () => {
  const home = join(testRoot, 'srv-reconfigure-write-failure');
  const fixture = createManagedServerFixture(home);
  writeFileSync(fixture.restartLog, '');
  const result = runLibrary(
    ['status'],
  ...
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /Could not install the candidate configuration/);
...
test('reconfigure catches SSH-style SIGHUP and restores files between atomic replaces', () => {
  const home = join(testRoot, 'srv-reconfigure-hup');
  const fixture = createManagedServerFixture(home);
  writeFileSync(fixture.restartLog, '');
  const result = runLibrary(
    ['status'],
    String.raw`
      ...
      mv() {
        move_count=$((move_count + 1))
        command mv "$@" || return
        [[ "$move_count" -ne 1 ]] || kill -HUP "$$"
      }
      reconfigure_server --client-to-client
    `,
  );

  assert.equal(result.status, 129, `stderr:\n${result.stderr}`);
  assert.doesNotMatch(result.stderr, /unbound variable/);
  assert.equal(readFileSync(fixture.config, 'utf8'), fixture.originalConfig);
  assert.equal(readFileSync(fixture.env, 'utf8'), fixture.originalEnv);
  assert.equal(readFileSync(fixture.restartLog, 'utf8'), '');
...
test('reconfigure refuses an unterminated managed block before changing live files', () => {
  const home = join(testRoot, 'srv-reconfigure-malformed-block');
  const fixture = createManagedServerFixture(home);
  const malformed = `${fixture.originalConfig}# BEGIN qp-tunnel-cli peer-mesh\ndisable-dco\n`;
  writeFileSync(fixture.config, malformed);
  writeFileSync(fixture.restartLog, '');
  const result = runLibrary(
    ['status'],
    String.raw`
  ...
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /Could not render the candidate server config/);
  assert.equal(readFileSync(fixture.config, 'utf8'), malformed);
  assert.equal(readFileSync(fixture.env, 'utf8'), fixture.originalEnv);
  assert.equal(readFileSync(fixture.restartLog, 'utf8'), '');
});

test('reconfigure can remove only the userspace peer-mesh directives', () => {
  const home = join(testRoot, 'srv-reconfigure-disable');
  const fixture = createManagedServerFixture(home, true);
  writeFileSync(
    fixture.config,
    fixture.originalConfig.replace(
      '# operator note must survive\n',
      '# operator note must survive\n' +
        'ignore-unknown-option disable-dco # operator compatibility\n' +
        'disable-dco # operator stability\n',
    ),
  );
  writeFileSync(fixture.restartLog, '');
  const result = runLibrary(
    ...
    String.raw`
      ...
      server_is_running() { return 0; }
      restart_running_server() { echo restart >> "${fixture.restartLog}"; }
      reconfigure_server --no-client-to-client
    `,
  );
  assertOk(result);
  ...
  const config = readFileSync(fixture.config, 'utf8');
  const env = readFileSync(fixture.env, 'utf8');
  assert.doesNotMatch(config, /^ignore-unknown-option disable-dco$/m);
  assert.doesNotMatch(config, /^disable-dco$/m);
  assert.doesNotMatch(config, /^client-to-client$/m);
  assert.match(config, /^ignore-unknown-option disable-dco # operator compatibility$/m);
  assert.match(config, /^disable-dco # operator stability$/m);
  assert.match(config, /^port 443$/m);
  assert.match(env, /^QP_OPEN_CLIENT_TO_CLIENT='false'$/m);
  assert.match(env, /^QP_OPEN_FUTURE_FIELD='preserve-me'$/m);
  assert.equal(readFileSync(fixture.restartLog, 'utf8'), 'restart\n');
});
