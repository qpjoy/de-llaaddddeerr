...
Outputs are written below `out/electron-builder/`. Unsigned local builds may
need quarantine removed before opening:

```bash
xattr -cr "out/electron-builder/mac-arm64/MX HDO.app"
open "out/electron-builder/mac-arm64/MX HDO.app"
```

electron-builder signs automatically when valid signing credentials are
available. On a macOS build machine, install a valid `Developer ID Application`
certificate in Keychain or provide `CSC_LINK` and `CSC_KEY_PASSWORD`. Set
`HDO_NOTARIZE=1` plus Apple notarization credentials when notarization should
run during packaging.
...
```text
out\electron-builder\MX HDO-0.1.0-win-x64.zip
out\electron-builder\MX HDO-0.1.0-win-x64.exe
```

electron-builder signs Windows artifacts automatically when `CSC_LINK` and
`CSC_KEY_PASSWORD` are set. If macOS and Windows use different certificates,
set `WIN_CSC_LINK` and `WIN_CSC_KEY_PASSWORD` for the Windows certificate.
...
- `electron-builder.config.cjs` keeps `asar: false` so the marketplace seed
  plugin can be installed from a real filesystem directory.
