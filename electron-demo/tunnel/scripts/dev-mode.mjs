#!/usr/bin/env node
/**
 * Switch electron-demo/tunnel between local development packages and
 * published npm packages.
 *
 * local: used by `pnpm dev`; packs the current workspace tunnel/market
 *        packages into .local-packs and installs from those tarballs.
...
const WORKSPACE_PACKS = [
  ...
  { name: '@qpjoy/electron-core-mihomo', dir: 'electron-plugin/packages/electron-core-mihomo' },
...
function buildWorkspaces() {
  ...
  shell('pnpm --filter @qpjoy/electron-core-mihomo build', resolve(REPO_ROOT, 'electron-plugin'));
...
if (desired === 'npm') {
  const npmPackageShape =
    ...
    !pkg.pnpm?.overrides;
  const alreadyNpm = npmPackageShape && npmDepsInstalled();
  ...
  console.log('-> switching electron-demo/tunnel to npm mode');
...
console.log('-> switching electron-demo/tunnel to local mode');
